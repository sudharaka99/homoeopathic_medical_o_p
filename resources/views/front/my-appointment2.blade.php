...
<div class="modal fade" id="paymentModal" tabindex="-1" aria-labelledby="paymentModalLabel" aria-hidden="true">
    <div class="modal-dialog modal-dialog-centered">
        <div class="modal-content">
            ...
            <form action="{{ route('patient.complete.payment') }}" method="POST" id="paymentForm">
                @csrf
                ...
                <div class="modal-body">
                    <div class="alert alert-info">
                        ...
                        <p class="mb-1"><strong>Appointment ID:</strong> <span id="paymentAppointmentId">Loading...</span></p>
                        <p class="mb-1"><strong>Amount:</strong> RS <span id="paymentAmount">Loading...</span></p>
                        <p class="mb-0"><strong>Doctor:</strong> <span id="paymentDoctorName">Loading...</span></p>
                    </div>
    ...
    document.querySelectorAll('.pay-btn').forEach(button => {
        button.addEventListener('click', async function() {
            ...
            try {
                ...
                if (data.success) {
                    $('#paymentDoctorName').text('Dr. ' + data.appointment.doctor_name);
                    await initializeStripePaymentForAppointment(appointmentId, fee);
...
// Initialize Stripe Payment for existing appointment
async function initializeStripePaymentForAppointment(appointmentId, amount) {
    try {
        // Create payment intent
        const response = await $.ajax({
            url: '{{ route("patient.create.appointment.payment.intent") }}',
            type: 'POST',
            data: {
                _token: '{{ csrf_token() }}',
        ...
        if (response.success) {
            ...
            $('#payment_stripe_payment_intent_id').val(response.paymentIntentId);
